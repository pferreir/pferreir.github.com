...
module Jekyll
  class WikipediaTag < Liquid::Tag
    ...
    def cleanup(doc)
      ...
      ['.unicode', '.reference', '.noprint'].each do |cls|
        description.css(cls).remove()
      end
    ...
    def get_cached_article(article)
      ...
      cache_file = get_cache_file_for article
    ...
    def get_cache_file_for(article)
      bad_chars = /[^a-zA-Z0-9\-_.]/
      article   = article.gsub bad_chars, ''
      md5       = Digest::MD5.hexdigest "#{article}"
      File.join @cache_folder, "#{article}.cache"
    end
  end
end
